...
__global__ void histogram256Kernel(uint *d_PartialHistograms, uint *d_Data, uint dataCount)
{
    ...
    __shared__ uint s_Hist[HISTOGRAM256_THREADBLOCK_MEMORY];
		// threadIdx.x >> LOG2_WARP_SIZE(5U) = warpIdx
    uint *s_WarpHist= s_Hist + (threadIdx.x >> LOG2_WARP_SIZE) * HISTOGRAM256_BIN_COUNT;

    //Clear shared memory storage for current threadblock before processing
#pragma unroll
    // (HISTOGRAM256_THREADBLOCK_MEMORY / HISTOGRAM256_THREADBLOCK_SIZE) = 
		// = (WARP_COUNT * HISTOGRAM256_BIN_COUNT) / (WARP_COUNT * WARP_SIZE) = 8
    ...
    //const uint tag = threadIdx.x << (UINT_BITS - LOG2_WARP_SIZE);	// threadIdx.x << (32-5)
